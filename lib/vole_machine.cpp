...
string decimal_to_hex(int decimal) {
  ...
  // Logic: divide the number by 16 and take the remainder as index in
  // hex_digits
  while (decimal > 0) {
...
unsigned char float_fix(unsigned char f) {
  ...
  // if mantissa doesn't have 1 in the first bit AND Exponent has at least one
  // bit
...
// STORE the bit pattern found in register R in the memory cell whose address is
// XY If you store in memory cell 00 print to screen
...
// complement representations or in float representations (1.3.4)
unsigned char VM::add(unsigned char S, unsigned char T, bool is_float) {
  if (is_float) {
    ...
    if ((first_number & 0b0'111'0000) > (second_number & 0b0'111'0000)) {
      ...
      temp_man >>=
          ((first_number & 0b0'111'0000) - (first_number & 0b0'111'0000)) >> 4;
    ...
    if ((first_number & 0b1'000'0000) == (second_number & 0b1'000'0000)) {
      // add two Mantissas
      unsigned short temp =
          (first_number & 0b0'000'1111) + (second_number & 0b0'000'1111);
      // overflow handle
...
void VM::operate(bool _steps) {
  ...
  while (run) {
    ...
    case 0x4:
      ...
      break;
    case 0x5:
      registers[reg] = add(memory[PC].get_value() >> 4,
                           memory[PC].get_value() & 0b00001111, false);
      break;
    case 0x6:
      registers[reg] = add(memory[PC].get_value() >> 4,
                           memory[PC].get_value() & 0b00001111, true);
      break;
    case 0x7:
      registers[reg] =
          OR(memory[PC].get_value() >> 4, memory[PC].get_value() & 0b00001111);
      break;
    case 0x8:
      registers[reg] =
          AND(memory[PC].get_value() >> 4, memory[PC].get_value() & 0b00001111);
      break;
    case 0x9:
      registers[reg] =
          XOR(memory[PC].get_value() >> 4, memory[PC].get_value() & 0b00001111);
      break;
    case 0xA:
...
void VM::read_file() {
  ...
  while (true) {
    cout << "Please insert where does program start in Hexadecimal (leave "
            "empty for default 0x10):"
         << endl;
    string input;
    getline(cin, input);
    if (input.empty()) {
  ...
  for (short i = 0; !instructions_file.eof() && ((PC + 2 * i) < 255); ++i) {
    ...
    if (input.size() != 4 || !is_hex(input) || input[0] > 'D' ||
        input[0] == '0') {
      if (!input.empty()) {
...
short VM_UI::ReadUserChoice(const short &From, const short &To) {
  ...
  do {
    ...
    else if (all_of(Input.begin(), Input.end(),
...
void VM_UI::ShowMainMenu() {
  ...
  while (isRunning) {
    ...
    enMainMenuOption choice =
        static_cast<enMainMenuOption>(ReadUserChoice(1, 4));
